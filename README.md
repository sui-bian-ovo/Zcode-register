...
- `Zcode-register-v2.0 [20260708]`：网站的验证码升级，程序自动过验证码的难度极大，因此使用半主动的方式，当验证码出现时，程序会开始等待人工拖动，并每`3`秒检查一次结果，当检查到验证码已完成后，便会自动往下进行。也就是说，需要人工拖动滑块，直达网页上出现**验证通过**即可。注意不需要人工点击登录按钮，程序会自动进行。
